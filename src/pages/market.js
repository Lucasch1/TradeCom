    ...
    <>
      ...
      <section className="h-screen py-20 bg-white">
        <div className="w-full max-w-screen-2xl h-20 mx-auto px-20 flex items-center justify-between text-amber">
          <div className="m-2 p-2 w-20 rounded-full bg-amber/75 text-white flex justify-center">
            Filter
          </div>
          <div className="flex justify-center">
            <input className="px-4 h-10 rounded-l-full bg-amber/25 focus:border-amber focus:border-2 focus:outline-none"></input>
            <button className="bg-amber/75 text-white p-2 h-10 rounded-r-full hover:bg-amber active:bg-amber/75">
              Search
            </button>
          </div>
        ...
        <div className="py-10 text-white">
            <div className="flex justify-center py-6 text-main/75 text-3xl font-bold">Commodities Market</div>
                <div>
                    <div className="w-full mx-auto px-20 max-w-screen-2xl grid grid-cols-3 gap-5">
                    {[...Array(8)].map((_, index) => (
                        <div
                        key={index}
                        className={`p-3 h-20 bg-main/75 rounded-lg flex flex-row items-center ${
